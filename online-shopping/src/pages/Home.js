...
<>
  ...
  <aside className="w-52 fixed left-0 top-16 h-screen shadow-md  p-10">
    ...
    <Radio.Group onChange={(e)=>setRadio(e.target.value)}>
      ...
      <button class=" w-auto bg-[#1569C7] rounded   text-white font-bold py-2 px-2 "
...
return (
  ...
  //   className="product-link">
    ...
    <div className="w-[80%] pb-4 rounded bg-white overflow-hidden shadow-lg hover:shadow-2xl ease-in duration-200 ease-out duration-200 " key={c._id}>
      ...
      <button class=" w-auto bg-red-600 text-[12px]  text-white font-bold py-1 px-1 " onClick={()=>navigate(`/product-details/${c.slug}`)}>
  More Details
  </button>
  <button class=" w-auto bg-yellow-600 text-[12px] text-white font-bold py-2 px-2 " onClick={()=>{setCart([...cart,c]);
